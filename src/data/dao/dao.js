...
const t5 = TABLE_NAMES.TENANTS_TABLE_NAME;
const t6 = TABLE_NAMES.USERS_TABLE_NAME;
...
function parseRowsToTenantsObj(rows) {
	try {
		const allTenants = {};
		...
		return Object.values(allTenants);
	} catch (e) {
		throw Error(`Error parsing rows to tenants object -> ${e}`);
	}
}

function parseRowsToUsersObj(rows) {
	try {
		const allTenants = {};
		...
		throw Error(`Error parsing rows to users object -> ${e}`);
	}
}
...
export async function deleteEndpoint(obj) {
	try {
		...
		const getRelatedRelsSQL = `SELECT id from ${t2} WHERE endpoint_id = ?`;
		4;
...
export async function getTenants(filter) {
	try {
		...
		console.log(ssql);
		const rows = await makePromiseQuery(ssql);
		const allTenants = parseRowsToTenantsObj(rows);
		console.log(allTenants);
		return allTenants;
	} catch (e) {
		throw `Error retrieving tenants -> ${e}`;
	}
}
...
export async function getUsers(filter) {
	try {
		...
		console.log(ssql);
		const rows = await makePromiseQuery(ssql);
		const allUsers = parseRowsToUsersObj(rows);
		console.log(allUsers);
		return allTenants;
	} catch (e) {
		throw `Error retrieving users -> ${e}`;
	}
}
